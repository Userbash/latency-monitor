...
const createWindow = () => {
    ...
    mainWindow.webContents.on('context-menu', (event) => {
        // Keep UI clean in release mode and prevent raw Chromium menu entries.
...
app.whenReady().then(() => {
    ...
    ipcMain.on('start-network-test', async (event, profile) => {
        ...
        try {
            ...
            event.reply('test-error', e.message);
        }
    });
    ...
    ipcMain.handle('capture-screenshot', async () => {
        ...
        const defaultPath = path.join(app.getPath('pictures'), `esports-network-monitor-${Date.now()}.png`);
